        ...
        fire: function(...args) {
            ...
            function eventHandler(event) {
                if(event.room_id !== CHAT.CURRENT_ROOM_ID || // event is not in this room
                   ...
                   event.user_id !== 8149646) return;        // event is not from NATOBot

                const content = document.createElement('div');
                content.innerHTML = event.content;

                if(!/^\[ Belisarius/.test(content.textContent.trim())) return; // event is not a report
...
script.textContent = `(${ BelisariusControls.toString() })();`;
